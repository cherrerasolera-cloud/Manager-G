      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">
             {activeTab === 'repository' ? 'Consolidated Compliance Ledger' : 'Generator Profile'}
          </h1>
          <p className="text-gray-500">
             {activeTab === 'repository' 
                ? 'Unified view of regulatory compliance and collection evidence.' 
                : 'Infrastructure management, maintenance logs, and discharge characterization.'}
          </p>
        </div>
        <div className="flex bg-gray-100 rounded-lg p-1 border border-border">
          <button 
            onClick={() => setActiveTab('repository')}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'repository' ? 'bg-white text-secondary shadow-sm' : 'text-gray-400 hover:text-gray-900'}`}
          >
            Monthly Reports
          </button>
          <button 
            onClick={() => setActiveTab('profile')}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'profile' ? 'bg-white text-secondary shadow-sm' : 'text-gray-400 hover:text-gray-900'}`}
          >
            Infrastructure Data
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                 ...
                 <table className="w-full text-sm text-left">
                    <thead className="text-xs text-gray-500 uppercase bg-gray-50 border-b border-border">
                      <tr>
                        ...
                        <th className="px-6 py-3">Amount (Kg)</th>
                          ...
                          <td className="px-6 py-4">
                             ...
                             {report.status === 'UPLOADED' && (
                               <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700 border border-amber-200">
                                 <FileText size={12} /> Pending Audit
                               </span>
                             )}
             ...
             <div className="space-y-6">
                <div className="bg-surface border border-border rounded-xl p-6 shadow-sm">
                    <div className="flex items-center gap-3 mb-6 pb-4 border-b border-border">
                       ...
                       <div>
                           <h3 className="text-lg font-semibold text-gray-900">Establishment Information</h3>
                           <p className="text-sm text-gray-500">Official regulatory data and operational contacts.</p>
                       </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                        <ProfileField label="Legal Entity Name" value={MOCK_PROFILE.razonSocial} />
                        <ProfileField label="Tax ID / Registration" value={MOCK_PROFILE.nit} />
                        <ProfileField label="Registered Address" value={`${MOCK_PROFILE.direccion}, ${MOCK_PROFILE.municipio}`} />
                        <ProfileField label="Legal Representative" value={MOCK_PROFILE.repLegal} />
                        <ProfileField label="Industry Code (CIIU)" value={`${MOCK_PROFILE.ciiu} - ${MOCK_PROFILE.actividad}`} />
                        <ProfileField label="Administrative Permit" value={MOCK_PROFILE.actoAdministrativo} highlight />
                    </div>
                    ...
                    <div className="mt-6 pt-6 border-t border-border">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                             <div className="bg-gray-50 p-3 rounded-lg border border-border">
                                 <div className="text-xs text-gray-500 uppercase font-bold tracking-wider">System Administrator</div>
                             ...
                             <div className="bg-gray-50 p-3 rounded-lg border border-border">
                                 <div className="text-xs text-gray-500 uppercase font-bold tracking-wider">Operational Lead</div>
                ...
                <div className="bg-surface border border-border rounded-xl p-6 relative overflow-hidden shadow-sm">
                    ...
                    <div className="flex items-center gap-3 mb-6">
                       ...
                       <div>
                           <h3 className="text-lg font-semibold text-gray-900">Infrastructure Maintenance Plan</h3>
                           <p className="text-sm text-gray-500">Suggested cycle based on pretreatment type and waste volume.</p>
                       </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                         <div className="bg-gray-50 border border-border p-4 rounded-lg">
                             <span className="text-xs text-gray-500 block mb-1">Pretreatment System</span>
                             <div className="text-gray-900 font-bold">{MOCK_PROFILE.tipoTrampa}</div>
                             <div className="text-xs text-gray-500 mt-1">Capacity: {MOCK_PROFILE.capacidadTrampaLts} L</div>
                         </div>
                         <div className="bg-gray-50 border border-border p-4 rounded-lg">
                             <span className="text-xs text-gray-500 block mb-1">Service Interval</span>
                             <div className="text-gray-900 font-bold">Every {MOCK_PROFILE.frecuenciaMantenimientoDias} Days</div>
                         ...
                         <div className={`border p-4 rounded-lg flex flex-col justify-center ${
                             ...
                             <span className={`text-xs font-bold uppercase mb-1 ${
                                 ...
                                 {maintenanceStatus.isOverdue ? 'Overdue!' : 'Next Maintenance'}
                             </span>
                             <div className="text-gray-900 font-bold text-lg">{maintenanceStatus.next}</div>
                             <div className="text-xs text-gray-600 mt-1">
                                 {maintenanceStatus.isOverdue 
                                    ? `Delayed by ${Math.abs(maintenanceStatus.daysRemaining)} days` 
                                    : `In ${maintenanceStatus.daysRemaining} days`}
                             </div>
                         </div>
                    </div>
                    ...
                    {maintenanceStatus.isDueSoon && (
                        <div className="flex items-center gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-sm mb-4 animate-in fade-in">
                            ...
                            <span>Warning: Maintenance is due soon. Contact your authorized collector to schedule suction.</span>
                        </div>
                    )}
                ...
                <div className="bg-surface border border-border rounded-xl p-6 shadow-sm">
                    <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-lg font-semibold text-gray-900">Wastewater Characterization</h3>
                                <p className="text-sm text-gray-500">Discharge parameter monitoring and legal compliance.</p>
                            </div>
                        </div>
                                ...
                                <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-4">
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            <h4 className="text-gray-900 font-medium">Report: {report.date}</h4>
                                            {report.status === 'Compliant' ? (
                                                <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[10px] rounded border border-emerald-200 uppercase font-bold">Compliant</span>
                                            ) : (
                                                <span className="px-2 py-0.5 bg-red-100 text-red-700 text-[10px] rounded border border-red-200 uppercase font-bold">Non-Compliant</span>
                                            )}
                                        </div>
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <div className="text-center">
                                            <div className="text-[10px] text-gray-500 uppercase">FOG (mg/L)</div>
                                            <div className={`font-mono font-bold ${report.grasasAceitesMgL > 100 ? 'text-red-600' : 'text-gray-900'}`}>{report.grasasAceitesMgL}</div>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-[10px] text-gray-500 uppercase">TSS (mg/L)</div>
                                ...
                                {report.recommendations.length > 0 && (
                                    <div className="mt-3 pt-3 border-t border-border/50">
                                        <div className="flex items-start gap-2">
                                            ...
                                            <div>
                                                <span className="text-xs text-amber-600 font-bold uppercase block mb-1">Expert System Recommendations</span>
        ...
        <div className="lg:col-span-4 space-y-6">
             ...
             <div className={`border rounded-xl p-4 flex flex-col gap-3 ${isRegistered ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
               <div className="flex items-center gap-3">
                  ...
                  <h3 className={`font-bold text-sm ${isRegistered ? 'text-emerald-700' : 'text-red-700'}`}>
                      {isRegistered ? 'Officially Registered' : 'Not Registered'}
                  </h3>
               </div>
               <p className="text-xs text-gray-600 leading-relaxed">
                   {isRegistered 
                      ? 'Your establishment is verified in the national environmental generator database.' 
                      : 'WARNING: Your establishment is not listed as a waste generator. This may lead to fines.'}
               </p>
               {!isRegistered && (
                 ...
                 {COMPLIANCE_CHECKLIST.map((item) => (
                    <div key={item.id} className="flex items-start gap-3 p-2 rounded hover:bg-gray-50 transition-colors">
                       ...
                       <div>
                          ...
                          <span className="text-[10px] uppercase tracking-wider text-gray-400 bg-gray-100 px-1.5 py-0.5 rounded font-bold">{item.requiredFor}</span>
                       </div>
                    </div>
                 ))}
              </div>
           ...
           <div className="bg-gradient-to-br from-amber-50 to-white border border-amber-200 rounded-xl p-5 shadow-sm">
              <h4 className="text-amber-700 font-medium mb-2 text-sm">Need Regulatory Help?</h4>
              <p className="text-xs text-gray-600 mb-3">
                 Questions about environmental reporting? Contact our specialized audit support team.
              </p>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
           <div className="bg-white border border-border rounded-xl w-full max-w-md p-6 shadow-2xl">
              <div className="flex justify-between items-center mb-6">
                 <h3 className="text-lg font-bold text-gray-900">Edit Month: {selectedReport.month}</h3>
                 <button onClick={() => setSelectedReport(null)} className="text-gray-400 hover:text-gray-900"><X size={20} /></button>
              </div>
              
              <form onSubmit={handleUpdateReport} className="space-y-4">
                 <div>
                    <label className="block text-sm text-gray-600 mb-1">Manual Weight Override (Kg)</label>
                    <input 
                      type="number" 
                      required
                 ...
                 <div className="pt-4 flex gap-3">
                    <button type="button" onClick={() => setSelectedReport(null)} className="flex-1 py-2.5 bg-white border border-border rounded-lg text-gray-600 hover:text-gray-900 hover:bg-gray-50 transition-colors">Cancel</button>
                    <button type="submit" className="flex-1 py-2.5 bg-primary text-white font-semibold rounded-lg hover:bg-emerald-600 transition-colors">Commit Changes</button>
                 </div>
              </form>
           </div>
        </div>
...
const ProfileField: React.FC<{ label: string; value: string | number; highlight?: boolean }> = ({ label, value, highlight }) => (
    <div>
        <label className="block text-xs text-gray-500 uppercase font-bold tracking-wider mb-1">{label}</label>
